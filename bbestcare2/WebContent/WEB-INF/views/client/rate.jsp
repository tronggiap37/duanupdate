...
<sql:query dataSource="${snapshot}" var="result5">
         select * from Rate Order by Id DESC; 
      </sql:query>
...
.feedback {
	/* background-image: linear-gradient(120deg, #66a6ff 100%, #89f7fe 100%); */
...
<script src="https://kit.fontawesome.com/f5d5718dac.js" crossorigin="anonymous"></script>
</head>
...
<body>
	...
	<br>
	<form action="client/rated.htm" name="myForm">
		<div class="rate">
			<div class="feedback">
				<p>Mời bạn chọn sao để đánh giá</p>
				<div class="rating">
					<input type="radio" value="Rất hài lòng" name="rating"
						id="rating-5"> <label for="rating-5"></label> <input
						type="radio" value="Hài lòng" name="rating" id="rating-4">
					<label for="rating-4"></label> <input type="radio"
						value="Bình thường" name="rating" id="rating-3"> <label
						for="rating-3"></label> <input type="radio" value="Không tốt"
						name="rating" id="rating-2"> <label for="rating-2"></label>
					<input type="radio" value="Rất kém" name="rating" id="rating-1"
						checked="checked"> <label for="rating-1"></label>
          ...
          <ellipse transform="rotate(-148.804 180.87 175.82)"
									cx="180.871" cy="175.822" rx="28.048" ry="28.08" fill="#3e4347" />
    ...
    <ellipse transform="scale(-1) rotate(45 454 -421.3)" cx="174.5"
          ...
          <path
				...
				<div class="o-hide">
				<input name="fname"
					style="text-align: center; color: blue;border: 1px solid black;"
					placeholder="Khách hàng" value="${user.fullname}"
					required="required" readonly="readonly"> <br>
				</div>
				<input name="note" style="border: 1px solid black;text-align: center;width: 80%;" placeholder="Nội dung đánh giá"> <br>
				<button onclick="return validateForm();" class="btn btn-primary"
					style="background-color: #17449e;width: 30%;">Đánh giá</button>
				<div class="o-hide">
					<input type="text" id="name" name="nameCustomer"
						value="${user.phone}" />
				</div>
	</form>
	...
	<div style="width: 2px;height: 45vh;background-color: #ccc;float: left;"></div>
	 
	<div class="feedback" >
	<div style="text-align: center; font-size: 15px; font-style: italic;height: 175px;width: 80%;margin: auto;margin-top: 60px;">
	<div style="width: 40%;float: left;height: 175px;">
		<div style="width: 100%;height: 25px;margin-top: 8px;">
			<div style="margin: auto;width: 65%;height: 25px;">
				<div style="width: 20%;float: left;height: 25px;"><i style="color:#fd7e14;" class="fas fa-star"></i></div>
				<div style="width: 20%;float: left;height: 25px;"><i style="color:#fd7e14;" class="fas fa-star"></i></div>
				<div style="width: 20%;float: left;height: 25px;"><i style="color:#fd7e14;" class="fas fa-star"></i></div>
				<div style="width: 20%;float: left;height: 25px;"><i style="color:#fd7e14;" class="fas fa-star"></i></div>
				<div style="width: 20%;float: left;height: 25px;"><i style="color:#fd7e14;" class="fas fa-star"></i></div>
	...
	<div style="width: 60%;float: left;height: 175px;">
	
		<div style="width: 60%;margin-left:20px; height: 25px;margin-top: 8px;">
			<c:forEach var="RHL" items="${result.rows}">${RHL.RHL}</c:forEach> đánh giá.
		</div>
		<div style="width: 60%;margin-left:20px; height: 25px;margin-top: 8px;">
			<c:forEach var="HL" items="${result1.rows}">${HL.HL}</c:forEach> đánh giá.
		</div>
		<div style="width: 60%;margin-left:20px; height: 25px;margin-top: 8px;">
			<c:forEach var="BT" items="${result2.rows}">${BT.BT}</c:forEach> đánh giá.
		</div>
		<div style="width: 60%;margin-left:20px; height: 25px;margin-top: 8px;">
			<c:forEach var="KT" items="${result3.rows}">${KT.KT}</c:forEach> đánh giá.
		</div>
		<div style="width: 60%;margin-left:20px; height: 25px;margin-top: 8px;">
			<c:forEach var="RK" items="${result4.rows}">${RK.RK}</c:forEach> đánh giá. 
		</div>
	...
	<hr style="border-bottom: 1px solid #666666; width: 80%;">
	<br>
	
	<c:forEach var="n" items="${result5.rows}">
	<div style="width: 80%;margin: 0 auto;">
	<div class="starcontainer" >
 	 <div class="vl">
        <span class="vl-innertext"><img style="width: 30px;height: 30px;" alt="images" src="images/logo-01.png"></span>
      </div>
      
    <div class="row">
      <div class="col" >
      <h2 style="text-align: center;color: #17449e;font-size: 25px;">${n.NameCustomer}</h2>
      </div>
      <div class="col1">
      		<div style="width: 29%;float: left;">
	        	<p class="o-hide">${n.Id}</p>
				<p  style="font-size: 25px;font-weight: bold;padding-top: 10px;">${n.Star}</p>
	            <p style="font-size: 10px;">${n.Dated}</p>
	            <p class="o-hide">${n.Phone}</p>
            </div>
            <div style="width: 2px;height: 65px;background-color: #ccc;float: left;"></div>
            <div style="width: 69%;float: left;padding-top: 10px;">
            	<p style="font-size: 18px;padding-left: 30px;float: left;font-weight: bold;">Nội dung:</p>
            	<p style="font-size: 16px;padding-left: 10px;float: left;">${n.Note}</p>
            </div>
      </div>
    </div>
</div>
<div class="bottom-container" style="height: 5px;">
...
if (x == "") {
    alert("Bạn cần phải đăng nhập để đánh giá hệ thống !");
    return false;
  }}​
</script>
	<script>
